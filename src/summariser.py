...
def initialise_summariser():
    device = -1 # Force CPU as couldn't mine to work with GPU
...
def summarise_text(text_to_summarise, summariser, tokeniser):
    ...
    try:
        ...
        for i in range(num_chunks):
            ...
            chunk_summary_list = summariser(chunk_text, max_length=150, min_length=30, do_sample=False)
        ...
        if len(tokeniser.encode(combined_summary)) <= 200:
             return combined_summary

        final_summary_list = summariser(combined_summary, max_length=200, min_length=50, do_sample=False)
